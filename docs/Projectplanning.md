...
!!! note "Definitie"
	PERT (Program Evaluation and Review Technique) is een hulpmiddel voor de bedrijfsleiding bij de analyse en planning van projecten. Hierbij wordt gebruik gemaakt van een grafische voorstelling, het netwerk, om de samenhang tussen de verschillende werkzaamheden aan te geven. [](Projectplanning%5D)
...
from "500 Reading notes" AND (outgoing([](Projectplanning%5D))) sort authors 
```